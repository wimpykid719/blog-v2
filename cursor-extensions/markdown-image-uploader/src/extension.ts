...
const DEFAULT_UPLOAD_ROOT = "blog-images";
...
function getConfig() {
  ...
  return {
    uploadRootFolder: cfg
      .get<string>("uploadRootFolder", DEFAULT_UPLOAD_ROOT)
      .trim(),
...
function makeRandomId(len: number): string {
  // base64url is compact and URL-safe; trim to desired length.
  return randomBytes(Math.ceil((len * 3) / 4))
    .toString("base64url")
    .slice(0, len);
}

function sanitizeUploadRootFolder(root: string): string | null {
  const r = (root ?? "").trim();
  if (!r) return null;
  // Avoid accidental leading/trailing slashes that create double separators in object paths.
  return r.replace(/^\/+/, "").replace(/\/+$/, "");
}

function escapeMarkdownAltText(alt: string): string {
  // Minimal escaping to avoid breaking ![alt](url) syntax.
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("markdownImageUploader.paste", async () => {
      ...
      try {
        ...
        const uploadRootFolder = sanitizeUploadRootFolder(cfg.uploadRootFolder);
        if (!uploadRootFolder) {
          log.warn("uploadRootFolder is empty; showing error and fallback.");
          vscode.window.showErrorMessage(
            `Markdown Image Uploader: uploadRootFolder が空です。settings.json の "markdownImageUploader.uploadRootFolder" を設定してください（例: "${DEFAULT_UPLOAD_ROOT}"）。`,
          );
          await pasteFallback();
          return;
        }
        ...
        log.info(
          `Uploading: localPath=${localPath} uploadRootFolder=${uploadRootFolder} objectPath=${objectPath}`,
        );
        ...
        await vscode.window.withProgress(
          ...
          async () => {
            ...
            const alt = escapeMarkdownAltText(path.parse(localPath).name);
            const md = `![${alt}](${url})`;
