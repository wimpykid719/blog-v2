...
export default function Pagination({
  ...
  const itemDom = (type: PageType, key: string | number) => {
    const buttonDisabled =
      disabled ||
      (type.indexOf("ellipsis") === -1 &&
        (type === "next" || type === "last" ? page >= lastPage : page <= 1));
  ...
  return (
    <div className="flex justify-center flex-col items-center w-full">
      <ul className="flex sm:space-x-3 space-x-2 justify-center items-center max-w-96 w-full overflow-x-scroll whitespace-nowrap hidden-scrollbar">
        {itemList.map((item, index) => (
          <li key={typeof item === "number" ? item : item}>{items[index]}</li>
        ))}
      </ul>
    </div>
  );
